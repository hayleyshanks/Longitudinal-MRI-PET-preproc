...
function longitudinal_PET_preproc_step3_QC(N, study_path, pet_to_run, pet_orders,study_name, frames, mri_ref, field_str, withinSubThresh, pet_QC_prefix, varargin)
...
parse(pars, N, study_path, pet_to_run, pet_orders, study_name, frames, mri_ref, field_str, withinSubThresh, pet_QC_prefix, varargin{:});
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if strcmpi(mri_ref{1}, 'base') 
    %% need to filter by field strength here  
    good_mri = mri_mastersheet.RID(mri_mastersheet.summarizedTimepointFails ==0 & mri_mastersheet.FieldStrength == field_str );
elseif strcmpi(mri_ref{1}, 'avg')
    good_mri = mri_mastersheet.RID(mri_mastersheet.summarizedAvgFails ==0 & mri_mastersheet.FieldStrength == field_str);
else
...
for t=1:length(pet_to_run)    
            ...
            if ~strcmpi(mastersheet.ScanType(i), pet_to_run{t})
                ...
                if mastersheet.withinSubWarpQC(i) < withinSubThresh
                    ...
                    if isnan(mastersheet.betweenSubWarpQC(i))
                        ...
                        sub = sub(strcmp(sub.ScanType, pet_to_run{t}), :);
    ...
    if pars.Results.checkReg
        ...
        master_subset = mastersheet(strcmpi(mastersheet.ScanType, pet_to_run{t}) , :);
        master_subset = master_subset(master_subset.summarizedFails ~= 1 , :);
        if isempty(master_subset)
            continue
        end
