...
function longitudinal_PET_preproc_step2(N, study_path, pet_to_run, pet_orders, mri_ref, study_name, frames, roi_dir, ref_mask, avgPrefix,field_str, pvc_opt, varargin)
...
addRequired(pars, 'ref_mask', @iscell);
...
addParameter(pars,'computeSUVR',default_run,@isnumeric); 
...
parse(pars, N, study_path, pet_to_run, pet_orders, mri_ref, study_name, frames, roi_dir,  ref_mask, avgPrefix,field_str,pvc_opt,varargin{:});
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            for a=1:length(pet_to_run)
                ...
                if isempty(pet_table)
                    ...
                    for p=1:length(to_pvc)
                        ...
                        matlabbatch{mbatch}.spm.tools.petpve.spatial.PVEcorrection.PVEopts.fwhm_PSF = [8 8 8];
...
if pars.Results.createAvg
    ...
    for a=1:length(pet_to_run)
        ...
        for scan=1:height(table_to_use)
            ...
            % drop any empty cells (if QC has not been run, there may be some
            % people who failed MRI QC and therefore have not warped PET so
            % they'll be missing data)
            if ~ isempty(get_files(dir_path, [avgPrefix, '*.nii']))
        ...
        if length(all_images) < step_size
            ...
            num_runs = round(length(all_images)/step_size);
            first = 1;
            last = step_size;
            for fl=1:num_runs
