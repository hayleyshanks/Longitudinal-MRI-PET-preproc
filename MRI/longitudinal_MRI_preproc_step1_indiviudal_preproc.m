...
function longitudinal_MRI_preproc_step1_indiviudal_preproc(N, study_path, mri_to_run, study_name, avgMode, indMode, field_str, templatePath, CAT12_shoot_path,  coreg_image, varargin)
...
addRequired(pars, 'coreg_image', @ischar);
% optional inputs.
...
parse(pars, N, study_path, mri_to_run, study_name, avgMode, indMode,field_str, templatePath,  CAT12_shoot_path, coreg_image, varargin{:});
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if pars.Results.dicomRecon || pars.Results.reorientAC
    for subI=sub_to_run(N)
        ...
        for a=1:length(mri_to_run)
            % only process the data modality we're currently interested in
            ...
            if ~strcmpi(indMode{1}, 'all_times') && isnumeric(indMode{1}) && strcmpi(avgMode, 'off')
                % only processing a subset of the scan dates. 
                table_to_run = table_to_run(ismember(table_to_run.Order, indMode{1}), :);
            end   
            if ~isempty(table_to_run)
                bl_dir = fullfile(sub_dir, char(table_to_run.Date(1)), table_to_run.Modality{1}, table_to_run.ScanType{1},table_to_run.ScanName{1});
            else
                ...
                if pars.Results.dicomRecon
                    if isempty(get_files(scan_dir, '*.dcm'))
                        ...
                        if contains(scan_dir, 'PD_T2') 
                            ...
                            % reconstruct the dicoms and move them into a
                            % subdirectory
...
for subI=sub_to_run(N)
    ...
    for t=1:length(mri_to_run)
        if pars.Results.coreg
            ...
            % study population. This is set by the user
            ...
            for row=1:height(anat_table)
                ...
                if strcmp(voxel_check, 'reslice')
                    ...
                    % voxel sizes
                    mbatch = mbatch +1;
                    matlabbatch{mbatch}.spm.spatial.coreg.estwrite.ref = {coreg_image};
    ...
    for t=1:length(mri_to_run)
        if pars.Results.longitudinalRegistration
            ...
            elseif height(anat) > 1 && isequal(sum(anat.IsDuplicate), 0)
                ...
                % processed
                if isequal(length(unique(anat.Date)),1)
                    % don't need to make any longitudinal averages because they
